...
function MarlinRow({ test }: { test: MarlinTest }) {
  ...
  const hasReview = test.review_status === "submitted" || test.review_status === "draft";

  return (
    <div className="group rounded-xl border border-border bg-surface p-5 transition-all hover:border-accent/30 hover:bg-surface-raised">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 flex-col items-end gap-2">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {hasReview && (
              <Link
                href={`/reviewer/marlin/${test.id}`}
                className="rounded-md border border-green-500/30 bg-green-500/10 px-3 py-1 text-xs font-medium text-green-300 hover:bg-green-500/20"
              >
                View review
              </Link>
            )}
